...
with open("lists\color_rgb_word2vec.txt","r",encoding="utf-8") as f:
...
# FNN
class FNNRGBdim2RGB(torch.nn.Module):
    def __init__(self, input_size=100, hidden_size=30, output_size=3):
        super(FNNRGBdim2RGB,self).__init__()
...
if __name__ == '__main__':
    ...
    net = FNNRGBdim2RGB(D_in,H,D_out).to(device)

    if not os.path.exists("params/fnnRGBdim2rgb_net.pth"):
        ...
        torch.save(net.to(device).state_dict(),"params/fnnRGBdim2rgb_net.pth")
    else:
        net.load_state_dict(torch.load("params/fnnRGBdim2rgb_net.pth"))
        net.eval()
        new_rgb = net(train_torch.to(device))
        new_rgb = new_rgb.tolist()

        with open("lists\color_rgb_fnn.txt","w",encoding="utf-8") as f: 
