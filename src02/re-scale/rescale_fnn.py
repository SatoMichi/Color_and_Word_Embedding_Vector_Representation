...
def str2list(s):
    nums = re.findall(r"-?\d+\.\d+",s)
    return [float(Decimal(n[-4:])) for n in nums]
...
if __name__ == '__main__':
    with open("original/color_768_fnn.txt","r",encoding="utf-8") as f:
    ...
    new_vecs3 = min_max(new_vecs2)
    new_rgbs = np.array([np.round(rgb).tolist() for rgb in new_vecs3*255])
    
    with open("normalized/color_768_fnn_norm.txt","w",encoding="utf-8") as f:
