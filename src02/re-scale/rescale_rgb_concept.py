...
if __name__ == '__main__':
    with open("original/concept_emb_rgb.txt","r",encoding="utf-8") as f:
        color_list = f.readlines()
    color_list = [c for c in color_list if c]
    concepts = [c.split("$")[0] for c in color_list]
    vecs = np.array([np.array(str2list(c.split("$")[1])) for c in color_list])

    with open("original/color_emb_rgb.txt","r",encoding="utf-8") as f:
        color_list = f.readlines() 
    color_list = [c for c in color_list if c]
    colors = concepts+[c.split(":")[0] for c in color_list]
    rgbs = [str2rgb(c.split(":")[1].split("|")[1].split("$")[0]) for c in color_list]
    vecs = np.concatenate((vecs,np.array([np.array(str2list(c.split(":")[1].split("|")[1].split("$")[1])) for c in color_list])))
    ...
    with open("normalized/concept_all_emb_rgb_norm.txt","w",encoding="utf-8") as f:
        for c,v1 in zip(concepts,new_rgbs[:len(concepts)]):
            f.write(c+":"+str(tuple(v1))+"\n")
