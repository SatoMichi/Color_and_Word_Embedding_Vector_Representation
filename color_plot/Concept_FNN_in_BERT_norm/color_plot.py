...
for i in range(len(concepts)):
    ...
    nearest_rgbs = np.array([c2v[c] for c in nearest_colors])
    avg_rgb = nearest_rgbs.mean(axis=0).astype(int)
    print(avg_rgb)
    hex3 = rgb_to_hex(tuple(avg_rgb))
    print(concept, hex2, nearest_colors)
    
    plt.figure(figsize=(15,5))
    plt.suptitle("\""+concept+"\"",size=150,x=0.5,y=0.7)
    ax1 = plt.subplot(1,6,1)
    ax1.set_title("Color Predicted:")
    ...
    #ax2 = plt.subplot(1,7,7)
    #ax2.set_title("Avarage color:")
    #ax2.set_facecolor(hex3)
